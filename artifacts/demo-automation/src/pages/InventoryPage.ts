...
export class InventoryPage {
  ...
  async updateTitle(
    ...
    if (
      (await this.root.getAttribute("data-account")) !== account ||
      (await item.getAttribute("data-version")) !== preview.version ||
      preview.target.sku !== sku
    )
    ...
    const actual = (
      await (
        await uniqueVisible(changed.getByTestId("title"), "saved-title")
      ).textContent()
    )?.trim();
