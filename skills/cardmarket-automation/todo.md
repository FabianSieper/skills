...
- **Fix:**
  ...
  2. **Phase 2:** For each qualifying offer, navigate directly to its `cardUrl` via `page.goto()` (no page-local index needed). Apply and verify the seller filter, read sellers, compute `diff`.
...
- **Fix:** In the comparison loop, after each detail page we `page.goto(ownOffersUrl)` back to the exact own-offers URL we were on (saved before `openOffer`). After the page loop finishes, the browser is on the last page that was visited — which is correct for continued `goToNextPage()` calls. The caller (`info` or `nav.own-offers`) can re-navigate to page 1 if needed.
- **Status:** ✅ DONE

## Files modified
| File | Change |
|---|---|
| `src/actions/stock-market-comparison.action.ts` | Full rewrite: Phase 1 uses `extractOffers()` (proven pagination); Phase 2 navigates directly to each `cardUrl`; new params `offset`/`minPrice`/`maxPrice`/`minQty`/`sellers`/`sortResult`; new output fields `offset`/`hasMore`/`quantity`/`condition`/`language`/`diff`; `marketSellers` is now an array |
| `src/pages/CardDetailPage.ts` | Increased `submitSellerFilters()` timeouts from 200 ms to 30 s / 15 s |
| `src/actions/nav-filter.action.ts` | Added post-submit filter read-back; throws `POSTCONDITION_FAILED` on `condition`/`language`/`location` mismatch |
...
- **Fix:**
  ...
  2. `nav-filter.action.ts` — after `submitSellerFilters()` + `settleSellerList()`, read the active filter back via `readCurrentFilter()` and compare `condition`, `language`, `location` to the resolved target. Throws `POSTCONDITION_FAILED` (`filter-not-applied`) on mismatch.
  3. `stock-market-comparison.action.ts` — retry once after a mismatch, re-read the filter, and throw `POSTCONDITION_FAILED` (`filter-mismatch`) with card, expected, and actual values if the three critical filters still don't match.
