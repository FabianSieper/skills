...
const outputDescription =
  'start: { state, ready }; results: { state, query, count, cards }; ' +
  'detail: { state, card, url, filter, info, sellerCount, sellers }; ' +
  'versions: { state, card, versionsUrl, total, shown, minQuantity, artworks }';
...
function validateOutput(raw: unknown): InfoOutput {
  ...
  if (raw.state === 'start') {
    if (typeof raw.ready !== 'boolean') throw new AutomationError('POSTCONDITION_FAILED');
    return raw as unknown as InfoOutput;
  }
  if (raw.state === 'results') {
    if (typeof raw.query !== 'string' || typeof raw.count !== 'number' || !Array.isArray(raw.cards) || raw.count !== raw.cards.length || !raw.cards.every(isSearchCard)) throw new AutomationError('POSTCONDITION_FAILED');
    return raw as unknown as InfoOutput;
  }
  if (raw.state === 'detail') {
    if (typeof raw.card !== 'string' || typeof raw.url !== 'string' || !isResolvedSellerFilter(raw.filter) || !isCardInfo(raw.info) || typeof raw.sellerCount !== 'number' || !Array.isArray(raw.sellers) || raw.sellerCount !== raw.sellers.length || !raw.sellers.every(isSellerOffer)) throw new AutomationError('POSTCONDITION_FAILED');
    return raw as unknown as InfoOutput;
  }
  if (raw.state === 'versions') {
    if (typeof raw.card !== 'string' || typeof raw.versionsUrl !== 'string' || typeof raw.total !== 'number' || typeof raw.shown !== 'number' || typeof raw.minQuantity !== 'number' || !Array.isArray(raw.artworks) || raw.shown !== raw.artworks.length || !raw.artworks.every((a) => isArtwork(a, raw.minQuantity as number))) throw new AutomationError('POSTCONDITION_FAILED');
    return raw as unknown as InfoOutput;
  }
...
export const action: Action = {
  ...
  run: async (page: Page, input: Input): Promise<InfoOutput> => {
    ...
    if (state === 'start') return { state, ready: true };
    if (state === 'results') {
      const results = new SearchResultsPage(page);
      const cards = await results.extractCards(input.limit as number);
      return { state, query: await results.query(), count: cards.length, cards };
    }
    if (state === 'detail') {
      ...
      return { state, card: info.title, url: page.url(), filter, info, sellerCount: sellers.length, sellers };
    }
    ...
    return { state: 'versions', card: await versions.cardFromUrl(), versionsUrl, total, shown, minQuantity: minQty, artworks };
  },
  modulePath: import.meta.url,
  next: ['nav.search', 'nav.open', 'nav.versions', 'nav.artwork', 'nav.filter'],
};
