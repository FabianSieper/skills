...
function invalid(fn: () => unknown): void {
  assert.throws(
    fn,
    (e: unknown) => e instanceof AutomationError && e.code === "INVALID_INPUT",
  );
}
...
test("missing required field, unknown property and invalid types are rejected", () => {
  for (const input of [
    ...
    { query: "" },
    { query: "ABC", limit: 21 },
    { query: "ABC", active: "true" },
    { query: "ABC", tags: [1] },
    { query: "ABC", tags: ["1", "2", "3", "4"] },
    { query: "ABC", mode: "fuzzy" },
    null,
    [],
  ])
...
test("prototype-looking unknown keys do not bypass input allowlist", () => {
  invalid(() =>
    validateInput(fields, JSON.parse('{"query":"x","__proto__":{"x":1}}')),
  );
  invalid(() =>
    validateInput(fields, JSON.parse('{"query":"x","constructor":"bad"}')),
  );
});
test("field definitions are validated", () => {
  validateFields(fields);
  for (const value of [
    { bad: { type: "string", description: "", min: 2, max: 1 } },
    { bad: { type: "number", description: "Bad", enum: ["x"] } },
    { bad: { type: "integer", description: "Bad", default: 1.5 } },
  ])
    invalid(() => validateFields(value as Fields));
});
