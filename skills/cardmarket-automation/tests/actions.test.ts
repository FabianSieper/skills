...
test('validateInput happy + sad for stock.market-comparison', () => {
  ...
  assert.equal(code(() => validateInput(a.parameters, { offset: -1 })), 'INVALID_INPUT');
  assert.equal(code(() => validateInput(a.parameters, { location: 'klingon' })), 'INVALID_INPUT');
  assert.equal(code(() => validateInput(a.parameters, { sortResult: 'bogus' })), 'INVALID_INPUT');
  assert.equal(code(() => validateInput(a.parameters, { unknown: 1 })), 'INVALID_INPUT');
});

test('validateOutput happy + sad for stock.market-comparison', () => {
  const a = byId('stock.market-comparison');
  const seller = { seller: 'A', location: 'DE', condition: 'Excellent', language: 'English', price: '0,50 €', quantity: '5' };
  const ok = {
    state: 'own-offers',
    offset: 0,
    count: 1,
    hasMore: false,
    offers: [{ articleId: 1, card: 'Forest', price: '1,23 €', quantity: 1, condition: 'Excellent', language: 'English', marketFrom: '0,50 €', marketSellers: [seller], belowMarket: false, diff: -0.73 }],
    auth: auth({ loggedIn: true }),
  };
  assert.doesNotThrow(() => a.validateOutput(ok));
  ...
  assert.equal(code(() => a.validateOutput({ ...ok, hasMore: 'yes' })), 'POSTCONDITION_FAILED');
  assert.equal(code(() => a.validateOutput({ ...ok, offers: [{ articleId: 1, card: 'Forest', price: 5, quantity: 1, condition: 'Excellent', language: 'English', marketFrom: '0,50 €', marketSellers: [], belowMarket: false, diff: null }] })), 'POSTCONDITION_FAILED');
});
